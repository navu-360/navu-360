...
interface EnrolledTalentWithDetails extends OnboardingProgramTalents {
  User: User;
}

interface CourseWithTalents extends OnboardingProgram {
  onboardingProgramTalents: EnrolledTalentWithDetails[];
}

interface LearningPathWithCourses extends LearningPathCourse {
  course: CourseWithTalents;
}

export interface DetailedlearningPath extends LearningPath {
  moduleCourse: LearningPathWithCourses[];
}

export default function Program({ data }: { data: DetailedlearningPath }) {
  const talentsFromAllCourses = () => {
    // from data.moduleCourse
    const talents: EnrolledTalentWithDetails[] = [];
    data?.moduleCourse?.forEach((course) => {
      course?.course?.onboardingProgramTalents?.forEach((talent) => {
        talents.push(talent);
      });
    });
    ...
    <>
      ...
      <DashboardWrapper hideSearch>
        <div className="relative ml-[90px] mt-[0px] flex h-full flex-col-reverse items-start justify-start gap-8 pt-12 md:ml-[250px] lg:flex-row">
          <GoBack />
          <div className="flex w-[95%] flex-col gap-4 lg:w-[calc(100%_-_450px)]"></div>
          <div className="right-8 top-28 mr-0 flex w-[95%] flex-col overflow-y-auto text-tertiary lg:fixed lg:mt-0 lg:h-[80vh] lg:w-[350px]">
            <div className="flex flex-col gap-4 rounded-xl border-[1px] border-gray-400 p-4 text-tertiary">
              ...
              <div className="flex flex-col gap-1 pl-[2px]">
                <div className="flex items-center gap-2 px-2">
                  ...
                  <span className="text-xs font-medium">Total Chapters</span>
                </div>
                <div className="flex items-center gap-2 px-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="flex w-full flex-col gap-0">
              ...
              <div className="mt-0 flex flex-col gap-4 rounded-xl p-4 px-0 text-tertiary">
                {talentsFromAllCourses()?.length === 0 && (
                ...
                {talentsFromAllCourses()?.map(
                  (enrollment: EnrolledTalentWithDetails, i: number) => (
                    <motion.div
                      initial={{ y: 15 }}
                      transition={{
                        duration: 0.3,
                ...
                <p className="w-full text-center text-sm font-medium text-gray-500">
                  ...
                  {talentsFromAllCourses()?.length > 0 && "more "}
...
export const getStaticPaths = async () => {
  try {
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_API_BASE_URL_V1}paths/all`,
      {
...
export const getStaticProps = async ({
  ...
  try {
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_API_BASE_URL_V1}paths/${params.id}`,
      {
        headers: { "Accept-Encoding": "gzip,deflate,compress" },
      },
    );
